...
// AuthUser authenticates a user with the given username and password.
//
// Parameters:
...
func (l *OSAuth) VerifyPasswordHash(hash, password string) bool {
	...
	if strings.HasPrefix(hash, "$y$") {
		return CompareHashAndPassword([]byte(hash), []byte(password)) == nil // yescrypt.CompareHashAndPassword(hash, password) == nil
